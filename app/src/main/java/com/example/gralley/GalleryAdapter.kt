...
class GalleryAdapter(val galleryViewModel: GalleryViewModel):ListAdapter<PhotoItem,MyViewHolder>(DIFFCALLBACK){
    ...
    var footerViewStaus= DATA_STATUS_CAN_MODE
   override fun onCreateViewHolder(parent:ViewGroup, viewType: Int):MyViewHolder{
       ...
       if (viewType== NORMAL_VIEW_TYPE){
           ...
           holder=MyViewHolder( LayoutInflater.from(parent.context).inflate(R.layout.gallery_footer,parent,
               false).also {
    ...
    override fun getItemViewType(position: Int): Int {
        return if (position==itemCount-1)
            FOOTER_VIEW_TYPE else NORMAL_VIEW_TYPE
    }
    ...
    override fun onBindViewHolder(holder: MyViewHolder, position: Int){
        if (position==itemCount-1){
            with(holder.itemView){
                when(footerViewStaus){
                    DATA_STATUS_CAN_MODE->{
                    ...
                    DATA_STATUS_NETWORK_MODE->{
                        progressBar.visibility=View.GONE
                        textView.text="网络连接错误,点击重试"
                        isClickable=true
        ...
        with(holder.itemView){
            ...
            textViewfavor.text=photoItem.photoFavorites.toString()
